...
### Рекомендуемый процесс (2 команды)

```bash
# 1. Сборка образа
docker build --platform linux/amd64 -t europe-west1-docker.pkg.dev/docmost-484110/docmost-repo/docmost:latest .
...
### Рекомендуемый процесс: раздельная сборка и деплой

**Рекомендуется** выполнять сборку и деплой раздельно, чтобы избежать таймаутов при длительной сборке:
...
Это выполнит сборку образа локально. Время выполнения: **5-10 минут**.

#### Шаг 2: Деплой собранного образа
...
2. **Загрузка образа** в Artifact Registry
3. **Деплой в Cloud Run** с обновлением только образа

**Важно**: Все настройки инфраструктуры (переменные окружения, Cloud SQL, Redis, VPC) сохраняются автоматически.

### Альтернативный вариант: единый процесс

Можно выполнить сборку и деплой одной командой:

```bash
./deploy.sh
```

**Примечание**: Этот вариант может завершиться таймаутом при длительной сборке, поэтому рекомендуется использовать раздельный процесс.
...
### Пропуск сборки (используется в рекомендуемом процессе)

Если образ уже собран локально (рекомендуемый подход):

```bash
./deploy.sh --no-build
```

Пропустит этап сборки и сразу загрузит существующий образ в Artifact Registry, затем задеплоит в Cloud Run.
...
### Рекомендуемый workflow (раздельный процесс)
...
4. **Задеплойте образ**:
   ...
   ./deploy.sh --no-build
   ```
   Дождитесь завершения деплоя (~3-7 минут)
5. **Проверьте результат** на https://wiki.megatools.janado.de

### Альтернативный workflow (единый процесс)
...
3. **Запустите деплой**:
   ...
   ./deploy.sh
   ```
4. **Дождитесь завершения** (8-17 минут)
5. **Проверьте результат** на https://wiki.megatools.janado.de

**Примечание**: Единый процесс может завершиться таймаутом, поэтому рекомендуется использовать раздельный процесс.
...
### Раздельный процесс (рекомендуется)

- **Сборка образа**: 5-10 минут (зависит от размера проекта)
- **Загрузка образа**: 2-5 минут
- **Деплой в Cloud Run**: 1-2 минуты
- **Общее время**: ~8-17 минут (но можно контролировать каждый этап отдельно)

### Единый процесс

- **Сборка + загрузка + деплой**: 8-17 минут (может завершиться таймаутом)
